...
class PollData {
    ...
    private users: mm.MyMap<number, UserChoice> = new mm.MyMap<number, UserChoice>();
    ...
    print(full?: boolean) {
        ...
        var select = this.options.map(function() { return {val: 0, users: []}; });
        this.users.forEach(function(user, choice: UserChoice) {
            select[choice.val].val++;
            select[choice.val].users.push(choice.user);
        });
        
        var lines = [ this.title || 'no title poll' ];
        lines = lines.concat(this.options.map(function(s, i) {
            var proc = select[i].val * 100 / usersPoll || 0;
            proc = Math.round(proc * 100) / 100;
            var res = '/' + (i + 1) + ' ' + s + ', ' + select[i].val + ' (' + proc + '%)';
            if (full)
              res += '\n' + select[i].users.map(function(user: Telegram.Bot.User) {
...
class PollBot extends chatAggr.GroupChat {
    poll: PollData;
    ...
    onCommand(desc: cmddesc.CommandDesc, user?: Telegram.Bot.User) {
    ...
    private onNewPoll(desc: cmddesc.CommandDesc, user: Telegram.Bot.User) {
        if (this.poll)
    ...
    private onShow(desc: cmddesc.CommandDesc) {
      return this.getPoll().print(desc.getArgs() == 'full');
    }
    
    private onShowFull(desc: cmddesc.CommandDesc) {
      return this.getPoll().print(true);
    }
    
    private onAddOption(desc: cmddesc.CommandDesc) {
        var poll = this.getPoll();
        poll.addOption(desc.getArgs());
    }
    
    private onTitle(desc: cmddesc.CommandDesc) {
      var poll = this.getPoll();
    ...
    private tryToSelectSilent(desc: cmddesc.CommandDesc, user: Telegram.Bot.User) {
        try {
...
new runner.Runner(BOT_NAME, PollBot.create).run();